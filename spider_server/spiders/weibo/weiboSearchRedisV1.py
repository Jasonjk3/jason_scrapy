...
class WeiboSearchRedisV1Spider(RedisSpider):
    """
    redis slave 爬虫
    微博 weibo.cn
    微博高级搜索页
    """
    name = 'weiboSearchRedisV1'
    ...
    redis_key = "weiboHotRank:start_urls"
    redis_name = "weiboDetail"
    ...
    def make_requests_from_url(self, url):
        ...
        try:
            datas = json.loads(url)  # 从redis中取出是string类型，需转回dict对象
            keyword = f"%23{datas.get('title')}%23"  # %23是#的意思
            data_url = self.url_format.format(keyword, self.date_start, self.date_end)

        except Exception as e:
            self.logger.error('redis队列中取出的数据异常，请检查')
            raise e
        return Request(url=data_url, dont_filter=True, headers=self.headers,
                       callback=self.parse_tweet)  # 将对象传入meta.data中
...
if __name__ == '__main__':
    cmdline.execute("scrapy crawl weiboSearchRedisV1".split())
